...
- The static hero wordmark is an `<img>` with an ink SVG data URI (`wordmarkDataUri` in `lib/brand.ts`) instead of inline SVG, with width and height carrying the exact viewBox ratio. Chrome only counts images and text as LCP candidates; the claim lines first paint hidden in their masks and then move on the compositor, and whether that first paint had any visible area depended on the platform font metrics: on the Linux runner Lighthouse found no LCP at all. The wordmark paints with the first frame everywhere, so LCP is that frame (29 260 px² at 412 px wide, 40 to 150 ms observed).
...
- `heroSnap` (`components/motion/heroSnap.ts`, set up with Lenis): a wheel or touch gesture (Lenis `virtual-scroll`) or a scroll key inside the hero scrolls to the end of the hero (down) or to the top (up) in 2 s, cubic out (1.2 s quart out felt too fast to Dmytro), with Lenis locked, which also cancels touch moves while it runs. Programmatic scrolls (anchors, tests) and a dragged scrollbar stay plain scrubbing. All hero scenes remain scrubbed by scroll, so the change is exact both ways and a state left halfway is valid.
