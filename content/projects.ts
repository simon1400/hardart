...
export const FeaturedSchema = BaseSchema.extend({
  ...
  /** pixel size of the site screenshot (`pnpm media` prints it), required with `site`: no layout shift */
  siteWidth: z.number().int().positive().optional(),
  siteHeight: z.number().int().positive().optional(),
})
  .refine((p) => !p.site || (p.siteWidth && p.siteHeight), {
    message: 'site needs siteWidth and siteHeight',
    path: ['siteWidth'],
  })
...
const featuredData: Project[] = [
  {
    ...
    siteWidth: 1000,
    siteHeight: 2991,
    text: '10 years, 50+ cities, a quarter of a million visitors/year. All of the design is ours, plus the vendor sign up system and the in app voting that crowns the best burger.',
